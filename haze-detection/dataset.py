...
class HacXDataset(Dataset):
    def __init__(self, data_dir, split='train', transform=None):
        ...
        # Extract unique classes and create custom label mapping
        # Use specific order: 0=smoke, 1=haze, 2=normal
        unique_classes = list(set([sample['label'] for sample in self.samples]))
        self.classes = ['smoke', 'haze', 'normal']

        # Verify all expected classes are present
        for cls in unique_classes:
            if cls not in self.classes:
                raise ValueError(f"Unexpected class '{cls}' found in dataset. Expected: {self.classes}")

        # Create custom class-to-index mapping (LabelEncoder sorts alphabetically, which we don't want)
    ...
    def __getitem__(self, idx):
        ...
        # Encode label using custom mapping
